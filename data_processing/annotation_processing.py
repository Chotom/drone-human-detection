...
def clean_annotations_from_files(source_dir: str, result_dir: str,
...
def from_xywh_to_xywhn_format(source_dir: str, result_dir: str, img_source_dir: str, file_cols_names: list[str]):
    """
    Reads data from all files in the specified directory with columns separated by ',' format,
    and convert x_left, y_top, width, height (xywh) columns to x_center, y_center, scaled_width, scaled_height (xywhz)
    ...
    :param img_source_dir: Directory path with images (to read width and height of image).
    ...
    for filename in os.listdir(source_dir):
        ...
        annotations.clip(lower=0, inplace=True)
